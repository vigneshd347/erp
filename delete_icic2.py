...
try:
    with urllib.request.urlopen(req) as response:
        ...
        for acc in data:
            if 'id' in acc:
                del_id = acc['id']
                del_req = urllib.request.Request(f'{SUPABASE_URL}/rest/v1/bank_accounts?id=eq.{del_id}', headers=headers, method='DELETE')
            else:
                del_req = urllib.request.Request(f'{SUPABASE_URL}/rest/v1/bank_accounts?account_name=eq.{urllib.parse.quote(acc["account_name"])}', headers=headers, method='DELETE')
